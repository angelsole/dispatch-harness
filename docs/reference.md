...
| `HARNESS_GATE_LOCK_KEY` | Overrides the lock key, which defaults to the repo directory's basename so that `dashboard-workspace-1/olyxbase` and `dashboard-workspace-2/olyxbase` contend with each other — they share one local Postgres. Set it to separate two checkouts that genuinely have their own database, or to join two that share one under different names. | repo basename |
