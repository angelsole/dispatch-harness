...
| `implementer-provider` | First line. Which subscription the run is spending, as the run's `provider`. It is read here rather than inferred from the stage text because `implementing — Opus (Claude sub)` is written verbatim on a `zai` run too. | Absent ⇒ `result.json`'s `implementer_provider`; neither ⇒ empty, which the [ops console](wall.md#the-ops-console) draws as an honest dash. | `wall.test.sh` — "detail: the implementer provider surfaces from its pin"; "detail: an escalated run reports the provider it escalated to" |
...
| `metrics.usage.turns`, `metrics.implementer_num_turns` | The run's `turns`. The whole invocation's count wins over the implementer's; anything that is not a finite number ≥ 0 is `null`, and so is a run still in flight, which has no `result.json` yet. | `wall.test.sh` — "detail: turns surface from the invocation's usage"; "detail: turns fall back to the implementer's own count"; "detail: a run with no countable turns carries null, not zero" |
| `implementer_provider` | Fallback for the `implementer-provider` file. | `wall.test.sh` — "detail: the provider falls back to result.json" |
