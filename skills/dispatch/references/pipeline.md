...
Works in Codex (`$dispatch`) and Claude Code (`/dispatch`). Resolve
`HARNESS_DIR` from the environment first; all `~/.claude/harness/` paths below
mean that directory, with `~/.claude/harness/` as the default. Runs live under
its `runs/<TICKET>/`. Use your host's available file, shell, tracker and image
tools; Claude tool names are examples, not dependencies.

Use `dispatch doctor` for the selected planner, or `dispatch doctor --pipeline --repo <repo>` for execution dependencies. The station-wide `station.sh doctor` is an operator diagnostic, not a prerequisite for every task.
...
Read the target repo's applicable AGENTS.md and CLAUDE.md yourself. Use a
research subagent (Explore in Claude Code) only when your host supports it and
a wide multi-file investigation would otherwise cost you many reads; do
not delegate what a few tool calls of your own would answer, and never spawn one
to re-check work already done. Find: the root cause / insertion point, the
conventions that apply, and what "done" verifiably means. Do not design the implementation in detail — that is
...
**Show the brief(s) to the user and get approval before dispatching, unless
the user has already authorized that scope in this session.**
...
Submit through the CLI; it detaches the driver and returns the run ID:

```bash
dispatch run --id <TICKET> --repo <repo-path> --branch <branch-name> \
  --brief ~/.claude/harness/runs/<TICKET>/brief.md
```
...
stages — the implementer's calls, then the reviewer's `◆ codex` / `◆ claude`
lines),
`status.sh [RUN-ID]` (table / timeline), and `attach.sh <RUN-ID>` (step into the
worker session). Use the host's background completion event when it provides
one. A detached run's desktop/phone notification does not itself resume a Codex
turn: if your host has no completion event, give the user the run ID and the
command `status.sh <RUN-ID>`, and explain that `resume dispatch <RUN-ID>` in a
new or existing conversation continues at the verdict step. Avoid continuous
log polling. Multiple tickets may run in parallel (separate worktrees).

For `resume dispatch <RUN-ID>`, read that run's `result.json` (or its current
status when it is still running) and continue from that state; do not launch a
second attempt merely to reconnect.
...
- **ready / ready_local** — verify against the brief before promoting: read
  ...
  load-bearing. Attribute work per stage: `opus_head` in result.json marks the
  boundary — commits up to it are the implementer's, commits after it are the
  reviewing tier's fixes (`reviewer_model` says which tier; attribution lives
  only in harness metadata, never in the commits themselves).
  ...
  inside the worktree (deps and .env are already there). For `ready_local`, report the worktree and stop: publishing was not requested.
  For a draft PR that satisfies the brief: `gh pr ready <pr_url>`, then run `~/.claude/harness/cleanup.sh
...
- **driver_failed** — the driver process was killed mid-stage (SIGTERM/HUP/INT,
  ...
  spending another pass. Runs detach into their own session, so the launcher's
  lifetime does not affect them; the deliberate way to stop a run is
...
- empty — the run never got that far. A `no_evidence` value in an old
  result.json means review produced nothing to weigh; treat it as unreviewed.
