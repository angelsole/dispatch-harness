...
# lib/*.sh and profiles/*/profile.sh too: helpers and profiles use stage()
...
grep -hoE '(^|;)[[:space:]]*stage "[^"]+"' "$SRC/run-task.sh" "$SRC/sync-pr.sh" "$SRC"/lib/*.sh $(echo "$SRC"/profiles/*/profile.sh) \
