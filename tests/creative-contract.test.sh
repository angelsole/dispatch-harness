...
# contract is complete and loadable off this checkout. Read-only: sources
# visual.conf.sh in a subshell with a temp VISUAL_TMP, the way
# creative/visual-gate.sh does, and writes nothing.
#
...
for f in visual.conf.sh rubric.md bible.md palette.png proportions.md assets.json refs; do
...
CONF="$(VISUAL_TMP="$TMP" bash -c '. "$1" || exit 1
  for v in VISUAL_SERVER_CMD VISUAL_URL VISUAL_READY_JS VISUAL_REFS VISUAL_RUBRIC VISUAL_MAX_PURE_BLACK_PCT \
           VISUAL_MIN_LEGIBILITY VISUAL_MAX_FRAME_RMSE VISUAL_MIN_SSIM; do printf "%s=%s\n" "$v" "${!v-}"; done
