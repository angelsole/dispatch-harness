...
# (every HARNESS_*, the model/provider knobs, provider key path and PATH) plus
# HARNESS_SEAT — the OS user the run dispatches as, taken from HARNESS_OWNER
# (the service user scheduling on someone's behalf sets it explicitly). The
# wrapper launches run-task.sh through seat_exec, so although it is written
# mode 600 it holds no credential of any kind: the seat reads its own logins
# from its own home.
set -u
...
# No credential travels: identity is the seat, and the seat holds its own
# logins in its own home.
env_names() {
  { compgen -e 2>/dev/null | grep -E '^HARNESS_[A-Za-z0-9_]+$'
    printf '%s\n' HARNESS_DIR HARNESS_SEAT IMPLEMENTER_PROVIDER IMPLEMENTER_MODEL \
      IMPLEMENTER_EFFORT ZAI_API_KEY_FILE REVIEWER_MODEL REVIEWER_EFFORT PATH
  } | sort -u
}
...
write_wrapper() {  # $1 ticket, $2 repo, $3 branch, $4 fire epoch, $5 wrapper path
  ...
  # it: the wrapper sits in the shared run dir, so it is unreadable to the other
  # seats even though it holds no credential.
  : > "$5" || return 1
...
# Armed for $(human_time "$4"). Mode 600: it sits in the shared run dir, though
# the snapshot below holds no credential — the seat carries its own logins.
set -u
...
echo "[schedule] $(date '+%Y-%m-%d %H:%M:%S') firing $TICKET as $HARNESS_SEAT"
# seat_exec replaces the child bash with the run, so this stays the wrapper's
# child: the exit status below and the bootout after it both still happen.
bash -c '. "$1/lib/common.sh"; umask 002; seat_run "$2" "$3" "$4" "$5" "$6"' \
  seat-launch "$HARNESS_DIR" "$HARNESS_SEAT" "$RUN_TASK" "$TICKET" "$REPO" "$BRANCH"
rc=$?
...
arm() {  # $1 ticket, $2 repo path, $3 branch, $4 when
  ...
  echo "[schedule]   seat    $HARNESS_SEAT"
  echo "[schedule]   agent   $(label_for "$ticket")"
  echo "[schedule]   wrapper $wrapper (mode 600 — holds an env snapshot, no credentials)"
