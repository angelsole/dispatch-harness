...
| `frames.py` | Deterministic capture on the Playwright inside shot-scraper's venv: revision-pinned managed Chromium only, SwiftShader, frozen clock, seeded `Math.random`, an app-ready predicate. If the pinned browser is missing, the gate records `not_run` and the `npx playwright install chromium` remedy instead of rendering with an independently updated host browser. |
