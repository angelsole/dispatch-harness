...
| `frames.py` | Deterministic capture on the Playwright inside shot-scraper's venv: SwiftShader, frozen clock, seeded `Math.random`, an app-ready predicate. It prefers Playwright's pinned Chromium and falls back to the installed stable Chrome channel when an upgrade has left that versioned browser cache empty. |
