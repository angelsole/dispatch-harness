...
#
# Columns: run, arm, implementer model, status, gate rounds (e.g. fail,pass),
#          implementer/reviewer commit counts, +/- lines vs base, wall minutes.
#
# Runs predating the metrics instrumentation (result.json without a `metrics`
# object) render with blanks, never errors — every field falls back with jq's //.
set -u
...
# Emit the nine data fields one per line (blank when absent). Splitting on
...
TABLE_FMT='%-26s %-9s %-7s %-13s %-11s %5s %5s %6s %6s %8s\n'

if [ "$CSV" = 1 ]; then
  echo "run,arm,model,status,gate_rounds,opus_commits,codex_commits,insertions,deletions,wall_minutes"
else
  # shellcheck disable=SC2059
  printf "$TABLE_FMT" RUN ARM MODEL STATUS GATE OPUS CODEX +LN -LN WALL_MIN
fi
...
for f in "$RUNS"/*/result.json; do
  ...
  { read -r arm; read -r model; read -r status; read -r gates
...
EOF
  ...
  if [ "$CSV" = 1 ]; then
    ...
    printf '%s,%s,%s,%s,"%s",%s,%s,%s,%s,%s\n' \
      "$run" "$arm" "$model" "$status" "$gates" "$oc" "$cc" "$ins" "$del" "$wall_min"
  else
    # shellcheck disable=SC2059
    printf "$TABLE_FMT" "$run" "${arm:--}" "${model:--}" "${status:--}" \
